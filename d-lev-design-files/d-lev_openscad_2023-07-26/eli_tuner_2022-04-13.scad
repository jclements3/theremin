...
DEBUG=0;							// debug mode (show pcb & leds)
TEMPLATE=1;						// 0=normal; else template
//
F_W=(TINY)?102.0:155;			// face width
F_L=(TINY)?61.375:90;				// face length
F_H=6;							// face height
F_R=4.5;							// face radius
F_CA=30;							// face chamfer angle (from vertical)
F_CH=1;							// face chamfer
//
S_D=2.5;							// screw hole dia
...
//
SEG7_X=(TINY)?-17.721:-0.8936*25.4;	// 7-segment X loc
...
//
SEG7_WC=-1;						// 7-segment window clearance (ledge)
...
SEG7_WR=0.1;						// 7-segment window radius
...
LED_PX0=(TINY)?24.6421:1.258*25.4;	// pitch LED X center loc
...
if(TEMPLATE) projection(cut=false) tuner();
else tuner();
...
module seg7(x=0, y=0){
	translate([x,y,SEG7_H/2]) cube([SEG7_W,SEG7_L,SEG7_H], center=true);  // seg7
}

module pitch_leds(x=0, y=0){
	translate([x,y,0]){
		led(0,2*LED_PYD);
		//
